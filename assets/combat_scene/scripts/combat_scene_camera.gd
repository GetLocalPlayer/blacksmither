...
# Файл содержи в себе код фокусировки камеры на персонаже
# или персонажах таким образом, чтобы они помещались в 
# field of view камеры. Для этого используется рассчет
# габаритов персонажа. Персонаж должен иметь функцию
# get_aabb, которая вернет aabb, соответствующий персонажу.
...
func set_view_on_characters(characters, instantly: bool = false) -> void:
